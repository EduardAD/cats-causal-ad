...
def run_pcmci_inference(df: pd.DataFrame, maxlag: int = 5, alpha: float = 0.05):
    """
    Ejecuta la inferencia causal usando el algoritmo PCMCI sobre el DataFrame dado.
    ...
    # Se asume que las columnas corresponden a variables.
    data = df.values  # matriz de numpy
    var_names = list(df.columns)
    dataframe = TigDataFrame(data, var_names=var_names)

    # Configurar el test de independencia a usar (ParCorr en este caso)
